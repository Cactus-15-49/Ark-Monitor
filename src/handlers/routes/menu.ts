...
import { Delegate, UContext, Voter } from "../../interfaces";
import { coingecko_request } from "../../utils/coingecko";
import { BigIntToBString, BigIntToString } from "../../utils/utils";
...
export class menu {
    ...
    public balance = async (ctx: UContext) => {
        ...
        for (const db_wallet of ctx.user.voters) {
            ...
            if (coingecko !== undefined) {
                ...
                answer += `${(Math.round(value_btc * 100000) / 100000).toFixed(5)} BTC ($${(
                    Math.round(value_usd * 100) / 100
                ).toFixed(2)})\n`;
            } else {
                answer += `??? BTC ($???)\n`;
            }
        ...
        if (ctx.user.voters.length > 1) {
            ...
            if (coingecko !== undefined) {
                answer += `${(Math.round(total_value_btc * 100000) / 100000).toFixed(5)} BTC ($${(
                    Math.round(total_value_usd * 100) / 100
                ).toFixed(2)})`;
            } else {
                answer += `??? BTC ($ ???)`;
            }
    ...
    public last_transactions = async (ctx: UContext) => {
        ...
        if (ctx.user.states[0] === "Dmenu") {
            ...
            if (data.length === 1) {
                ...
                const transactions: Interfaces.ITransactionData[] =
                    await this.transactionHistoryService.findManyByCriteria({ address: data[0].address });
                let message = "";
                ...
                if (transactions.length <= 5)
                    keyboard = { reply_markup: { inline_keyboard: [[{ text: "1", callback_data: "a" }]] } };
                else
                    keyboard = {
                        reply_markup: {
                            inline_keyboard: [
                                [
                                    ...
                                    { text: "next", callback_data: `next_1_${data[0].address}` },
                                ],
                            ],
                        },
                    };
        ...
        for (let r = 0; r < data.length; r++) {
            row.push((data[r] as Voter).name || (data[r] as Delegate).username || data[r].address);
            if ((r + 1) % 2) {
                keyboard.push(row);
                row = [];
            }
    ...
    public price = async (ctx: UContext) => {
        const coingecko = await coingecko_request(this.get_coingecko_ticker());
        if (coingecko === undefined) {
            ctx.reply("There are problems with CoinGecko. Try again later.", {
    ...
    public rednodes = async (ctx: UContext) => {
        ...
        const rednodes: Map<string, number> = this.alerts_handler.get_missing_delegates();
        rednodes.forEach((missed, pkey) => {
            const red_delegate_wallet: Contracts.State.Wallet = this.wallets.findByPublicKey(pkey);
            const username: string = red_delegate_wallet.getAttribute("delegate.username");
            ...
            if (missed === 1) {
                ...
                if (ctx.user.delegates.some((delegate) => delegate.username === username)) {
                    red += `\n_+++YOU ARE RED+++ (${username})\n|___Missed blocks: ${missed} ~ ${Math.round(
                        days,
                    )} day(s)`;
                } else {
                    red += `\n_${username} is red. `;
        ...
        if (orange !== "") ctx.reply(`ORANGE NODES (${n_orange}).${orange}`);
        if (red !== "") ctx.reply(`RED NODES (${n_red}).${red}`);
        if (orange === "" && red === "") ctx.reply("No rednodes");
    };

    public voter_notifications = (ctx: UContext) => {
        const voters = ctx.user.voters;
        if (voters.length < 2) {
            ...
            ctx.reply("Choose the notification you want to turn on or off", {
            ...
            ctx.reply("Which wallet do you want to change the notifications?", {
    ...
    public settings = (ctx: UContext) => {
        ...
        let data: Delegate[] | Voter[];
        const extra = ["+Add"];
        if (ctx.user.states[0] === "Dmenu") {
            data = ctx.user.delegates;
            if (!ctx.user.voters.length) extra.push("+Add voter");
        } else {
            data = ctx.user.voters;
            if (!ctx.user.delegates.length) extra.push(`+Add ${this.get_delegate_name()}`);
        }
        extra.push("/Back");
        ...
        for (const line of extra) keyboard.push([line]);

        if (ctx.user.states[0] === "Dmenu")
            ctx.reply(`Which ${this.get_delegate_name()} do you want to modify?`, {
                reply_markup: Markup.keyboard(keyboard),
            });
        else ctx.reply("Which wallet do you want to modify?", { reply_markup: Markup.keyboard(keyboard) });
    };
    ...
    public delegates_info = async (ctx: UContext) => {
        ...
        for (const delegate of delegates) {
            ...
            const delegateAttribute: Contracts.State.WalletDelegateAttributes = wallet.getAttribute("delegate");
            ...
            const needed_delegates: Contracts.State.Wallet[] = this.alerts_handler
                .getDelegateRankList()
                .filter((delegate) => {
                    if (!rank) return false;
                    const delegate_rank = delegate.getAttribute("delegate.rank");
                    return (
                        Math.abs(delegate_rank - rank) === 1 ||
                        delegate_rank === milestone.activeDelegates ||
                        delegate_rank === milestone.activeDelegates + 1
                    );
                });
            ...
            if (wallet.hasAttribute("delegate.resigned")) {
                ...
                message += `VOTES: ${BigIntToBString(delegateAttribute.voteBalance, 2)} ${
                    this.network.client.token
                } (${AppUtils.delegateCalculator.calculateVotePercent(
                    wallet,
                    AppUtils.supplyCalculator.calculate(this.wallets.allByAddress()),
                )}%)\n`;

                const minor_rank = needed_delegates.find(
                    (delegate) => delegate.getAttribute("delegate.rank") - rank! === 1,
                );
                const greater_rank = needed_delegates.find(
                    (delegate) => delegate.getAttribute("delegate.rank") - rank! === -1,
                );
                const forge_rank = needed_delegates.find(
                    (delegate) => delegate.getAttribute("delegate.rank") === milestone.activeDelegates,
                );
                const forge_rank_less_1 = needed_delegates.find(
                    (delegate) => delegate.getAttribute("delegate.rank") === milestone.activeDelegates + 1,
                );
                if (rank! <= milestone.activeDelegates) {
                    if (forge_rank_less_1 !== undefined) {
                        message += `└ TO ${milestone.activeDelegates + 1}: `;
                        message += `${BigIntToBString(
                            forge_rank_less_1
                                .getAttribute<Utils.BigNumber>("delegate.voteBalance")
                                .minus(delegateAttribute.voteBalance),
                            2,
                        )} ${this.network.client.token}\n`;
                    }
                } else {
            ...
            if (last_block_id) {
                ...
                message +=
                    `Last block forged ${secondsToHms(
                        (current_height - last_block_height) * milestone.blocktime,
                    )} ago\n` +
                    `└ Height: ${BigIntToBString(last_block_height, 0, 0)}\n└ Timestamp: ${
                        AppUtils.formatTimestamp(last_block!.timestamp).human
                    }\n`;
                ...
                message +=
                    `\nTotal forged: ${BigIntToBString(
                        delegateAttribute.forgedFees.plus(delegateAttribute.forgedRewards),
                        2,
                    )} ${this.network.client.token}\n` +
        ...
        ctx.reply(message, {
            reply_markup: Markup.inlineKeyboard([Markup.callbackButton("Update", "update_delegateinfo")]),
        });
    };
    ...
    public delegate_notifications = (ctx: UContext) => {
        ...
        if (delegates.length < 2) {
            ...
            if (delegate.Votes === "OFF") {
                keyboard = [
                    ["Rednode: " + delegate.Missing, "Position: " + delegate.Position],
