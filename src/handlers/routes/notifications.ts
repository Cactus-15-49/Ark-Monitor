...
export class notifications {
    ...
    public change_notification_voter = (ctx: UContext) => {
        ...
        const text = ctx.text;
        const possible_notifications = [
            "Rednodes: " + voter.Rednodes,
            "Out of Forging: " + voter.Out_of_forging,
        ...
        if (text === possible_notifications[0]) {
            if (voter.Rednodes === "OFF") voter.Rednodes = "ON";
            else voter.Rednodes = "OFF";
            ctx.reply(`REDNODES turned ${voter.Rednodes}.`, { reply_markup: Markup.keyboard(keyboard()) });
        } else if (text === possible_notifications[1]) {
            if (voter.Out_of_forging === "OFF") voter.Out_of_forging = "ON";
            else voter.Out_of_forging = "OFF";
            ...
            if (voter.Transactions === "OFF") voter.Transactions = "ON";
            else voter.Transactions = "OFF";
            ctx.reply(`PAYMENTS turned ${voter.Transactions}.`, { reply_markup: Markup.keyboard(keyboard()) });
        }
        this.db.update_alerts_voter(
            ctx.chat_id,
            voter.address,
            voter.Rednodes,
            voter.Out_of_forging,
            voter.Transactions,
        );
    };
    ...
    public choose_delegate = (ctx: UContext) => {
        ...
        if (choice !== undefined) {
            ...
            if (choice.Votes === "OFF") {
                keyboard = [
                    ["Rednode: " + choice.Missing, "Position: " + choice.Position],
    ...
    public change_notification_delegate = (ctx: UContext) => {
        ...
        const text = ctx.text;
        const possible_notifications = [
            "Rednode: " + delegate.Missing,
            "Position: " + delegate.Position,
            ["Votes: " + delegate.Votes, "Votes: ON/Cap: " + delegate.Votes],
        ...
        if (text === possible_notifications[0]) {
            if (delegate.Missing === "OFF") delegate.Missing = "ON";
            else delegate.Missing = "OFF";
            ctx.reply(`REDNODE turned ${delegate.Missing}.`, { reply_markup: Markup.keyboard(keyboard()) });
        } else if (text === possible_notifications[1]) {
            if (delegate.Position === "OFF") delegate.Position = "ON";
            else delegate.Position = "OFF";
            ...
            if (delegate.Voters === "OFF") delegate.Voters = "ON";
            else delegate.Voters = "OFF";
            ctx.reply(`VOTERS turned ${delegate.Voters}.`, { reply_markup: Markup.keyboard(keyboard()) });
        }
        this.db.update_alerts_delegate(
            ctx.chat_id,
            delegate.username,
            delegate.Missing,
            delegate.Position,
