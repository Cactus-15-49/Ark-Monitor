...
export class settings {
    ...
    public delegate_add = (ctx: UContext) => {
        if (ctx.user.delegates.length <= this.get_base_delegates() + ctx.user.extra_delegates) {
            ctx.reply(`Insert your ${this.get_delegate_name()} username here`, {
                reply_markup: Markup.keyboard([["/Back"]]),
            });
            this.db.enter_menu(ctx.chat_id, ctx.user.states, "add");
        } else
            ctx.reply(`You have inserted the max number of ${this.get_delegate_name()}s. 
                            Contact @cactus_1549 if you need more.`);
    };
    ...
    public delegate_add_from_voter = (ctx: UContext) => {
        ...
        } else ctx.reply(`Furbacchione, you shouldn't be using that here :thinking:`);
    };

    public delegate_main = (ctx: UContext) => {
        const choice = ctx.user.delegates.find(
            (delegate) => delegate.username === ctx.text || delegate.address === ctx.text,
        );
        if (choice !== undefined) {
            this.db.enter_menu(ctx.chat_id, ctx.user.states, choice.address);
            ctx.reply(
                `You are modyfing the ${this.get_delegate_name()} ${choice.username}.\nChoose what you want to do.`,
                { reply_markup: Markup.keyboard([["delete"], ["/Back"]]) },
            );
        } else {
    ...
    public check_username = (ctx: UContext) => {
        ...
        try {
            ...
            ctx.reply(`This username is not registered as ${this.get_delegate_name()}. Try again. `, {
                reply_markup: Markup.keyboard([["/Back"]]),
            });
            return;
        }
        if (wallet.hasAttribute("delegate.resigned") && wallet.getAttribute("delegate.resigned"))
            ctx.reply(`You can't add resigned ${this.get_delegate_name()}s. Try again.`, {
                reply_markup: Markup.keyboard([["/Back"]]),
            });
        else if (ctx.user.delegates.some((delegate) => wallet.getAttribute("delegate.username") === delegate.username))
            ctx.reply(
                `You have already inserted this address as a ${this.get_delegate_name()}. Please insert another ${this.get_delegate_name()} username.`,
                { reply_markup: Markup.keyboard([["/Back"]]) },
            );
        else {
            ...
            ctx.reply(`Inserted ${username} as a ${this.get_delegate_name()}.`, {
                reply_markup: Markup.keyboard([["/Back"]]),
            });
        }
    };
    ...
    public delete_delegate = (ctx: UContext) => {
        ...
        ctx.reply(`Are you sure you want to delete ${delegate.username} (${delegate.address}).`, {
    ...
    public voter_add = (ctx: UContext) => {
        if (ctx.user.voters.length <= this.get_base_voters() + ctx.user.extra_voters) {
            ...
            this.db.enter_menu(ctx.chat_id, ctx.user.states, "add");
        } else ctx.reply("You have inserted the max number of voters. Contact @cactus_1549 if you need more.");
    };
    ...
    public check_address = (ctx: UContext) => {
        ...
        if (AddressOrUsername.length <= 20) {
            try {
                ...
                ctx.reply("Invalid wallet address. Try again or press /Back to go back.", {
                    reply_markup: Markup.keyboard([["/Back"]]),
                });
                return;
            }
        ...
        if (wallet.getPublicKey()) {
            if (ctx.user.voters.some((voter) => wallet.getAddress() === voter.address)) {
                ctx.reply("You have already inserted this address as a voter. Please insert another address.", {
                    reply_markup: Markup.keyboard([["/Back"]]),
                });
                return;
            }
            ...
            ctx.reply("Do you want to add a name for this address? Write it down or press /Continue", {
                reply_markup: Markup.keyboard([["/Continue"]]),
            });
    ...
    public add_name_settings = (ctx: UContext) => {
        const name: string = ctx.text;
        const pattern = /^[a-zA-Z0-9_ -]*$/;
        if (name.length < 1 || !pattern.test(name) || name.length > 25) {
            ctx.reply(
                "The name should be shorter than 25 characters and can only contain alphanumeric characters, spaces and - or _. Try with another name.",
                { reply_markup: Markup.keyboard([["/Continue"]]) },
            );
        } else if (ctx.user.voters.some((voter) => name === voter.name)) {
            ctx.reply("This name already exist. Try with another name.", {
                reply_markup: Markup.keyboard([["/Back"]]),
            });
        } else {
    ...
    public delete_address = (ctx: UContext) => {
        ...
        if (wallet.name !== undefined) message += `${wallet.name} (${wallet.address}).`;
        else message += `${wallet.address}.`;
        this.db.enter_menu(ctx.chat_id, ctx.user.states, "delete");
    ...
    private get_base_delegates() {
        const config_base_delegates = this.configuration.get("base_delegates");
    ...
    private get_base_voters() {
        const config_base_voters = this.configuration.get("base_voters");
