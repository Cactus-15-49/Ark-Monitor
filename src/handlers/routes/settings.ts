...
export class settings {
    ...
    public delegate_add_from_voter = (ctx: UContext)=>  {
        if (ctx.user.delegates.length == 0){
            ...
            ctx.reply(`Furbacchione, you shouldn't be using that here :thinking:`)
    }
    ...
    public check_username = (ctx: UContext)=>  {
        ...
        try{
            ...
            ctx.reply(`This username is not registered as ${this.get_delegate_name()}. Try again. `, {reply_markup: Markup.keyboard([["/Back"]])});
            return;
        }
        if (wallet.hasAttribute("delegate.resigned") && wallet.getAttribute("delegate.resigned")) ctx.reply(`You can't add resigned ${this.get_delegate_name()}s. Try again.`, {reply_markup: Markup.keyboard([["/Back"]])});
    ...
    public voter_add = (ctx: UContext)=>  {
        if (ctx.user.voters.length <= this.get_base_voters() + ctx.user.extra_voters){
            ...
            ctx.reply("You have inserted the max number of voters. Contact @cactus_1549 if you need more.")
    }
    ...
    public add_name_settings = (ctx: UContext)=>  {
        ...
        if (name.length < 1 || !pattern.test(name) || name.length > 25){
            ...
            ctx.reply("This name already exist. Try with another name.",  {reply_markup: Markup.keyboard([["/Back"]])})
    ...
    public confirm_change_name = (ctx: UContext)=>  {
        ...
        const pattern = /^[a-zA-Z0-9_ -]*$/;
        if (name.length < 1 || !pattern.test(name) || name.length > 25){
            ctx.reply("The name should be shorter than 25 characters and can only contain alphanumeric characters, spaces and - or _. Try with another name.",  {reply_markup: Markup.keyboard([["/Back"]])})
        }else if(ctx.user.voters.some((voter) => name ===  voter.name)){
            ctx.reply("This name already exist. Try with another name.",  {reply_markup: Markup.keyboard([["/Back"]])})
