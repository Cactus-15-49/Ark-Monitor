...
export class init {
    ...
    public init = (ctx: UContext) => {
        ...
        ctx.reply(
            `Welcome to the ${this.get_token()} Telegram bot.\nAre you a voter, ${this.get_delegate_name()} or both?\n\nTip: Always use the keyboard unless you get asked to write something!`,
            { reply_markup: Markup.keyboard([["voter", this.get_delegate_name()], ["both"]]) },
        );
    };
    ...
    public voter_check = (ctx: UContext) => {
        ...
        if (AddressOrUsername.length <= 20) {
            try {
                ...
                ctx.reply("INVALID USERNAME\nThis username does not exist. Try again or press /Back to go back.", {
        ...
        if (wallet.getPublicKey()) {
            ...
            ctx.reply(
                "Do you want to assign a name to this wallet so that you can recognize it better?\nWrite it now or hit the /Continue button to skip.",
                { reply_markup: Markup.keyboard(["/Continue"]) },
            );
            this.db.enter_menu(chat_id, ctx.user.states, `name/${wallet.getAddress()}`);
        } else {
            ctx.reply(
                "INVALID WALLET ADDRESS\nThis wallet address doesn't exist. Try again or press /Back to go back.",
                { reply_markup: Markup.keyboard(["/Back"]) },
            );
        }
    };
    ...
    public add_name = (ctx: UContext) => {
        ...
        if (name.length < 1 || !allowed_pattern.test(name) || name.length > 25) {
            ctx.reply(
                "INVALID NAME!\nThe name must be shorter than 25 characters and can only contain alphanumeric characters (a-z A-Z 0-9), spaces and - or _. Try with another name.",
                { reply_markup: Markup.keyboard(["/Continue"]) },
            );
        } else if (ctx.user.voters.some((voter) => name === voter.name)) {
            ctx.reply(
                "NAME ALREADY EXIST\nYou have already used this name for another address. Try with a different name.",
                { reply_markup: Markup.keyboard([["/Continue"]]) },
            );
        } else {
            ...
            if (ctx.user.states[1] === "voter") {
                ...
                ctx.reply(
                    `Please send your ${this.get_delegate_name()} username or press /Continue to continue without inserting any ${this.get_delegate_name()}.`,
                    { reply_markup: Markup.keyboard(["/Continue"]) },
                );
            }
        }
    ...
    public delegate_check = async (ctx: UContext) => {
        ...
        try {
            ...
            ctx.reply(
                `USERNAME DOESN'T EXIST\nThis username is not registered as a ${this.get_delegate_name()}. Try again or use the /Back button to go back.`,
                { reply_markup: Markup.keyboard(["/Back"]) },
            );
            return;
        }
        if (wallet.hasAttribute("delegate.resigned") && wallet.getAttribute("delegate.resigned"))
            ctx.reply(
                `You can't add resigned ${this.get_delegate_name()}s. Try again with a valid username or use the /Back button to go back.`,
                { reply_markup: Markup.keyboard(["/Back"]) },
            );
        else {
