...
export class Database{
    ...
    public connect() {
        mongoose.connect(`mongodb://127.0.0.1/${this.configuration.get("databaseName")}`, { useNewUrlParser: true, useUnifiedTopology: true, useFindAndModify: false, useCreateIndex: true });
    }
    ...
    public async create_voter(chat_id: number, address: string){
    ...
    public async create_delegate(chat_id: number, username: string, address: string){
    ...
    public async update_alerts_voter(chat_id: number, address: string, Rednodes: string, Out_of_forging: string, Transactions: string){
        await voters.findOneAndUpdate({chat_id, address}, {Rednodes, Out_of_forging, Transactions});
    }

    public async update_alerts_delegate(chat_id: number, username: string, Missing: string, Position: string, Votes: string, Voters: string ){
