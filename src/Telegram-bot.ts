...
export class Telegram_bot {
    ...
    public async start() {
        ...
        if (!token) {
            this.logger.error("Telegram Token not set. The bot will not answer to incoming messages.");
            return;
        }
        this.bot = new Telegraf<UContext>(token);
        ...
        this.bot.on("message", (ctx: UContext) => {
            if (ctx.message !== undefined && ctx.text !== undefined) this.message_handler.handle(ctx);
        });
